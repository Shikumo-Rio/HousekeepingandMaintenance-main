...
if ($format === 'excel') {
    exportToExcel($data, $filename, $startDate, $endDate, $generatedBy, $action);
} elseif ($format === 'pdf') {
 ...
 */
function exportToExcel($data, $filename, $startDate, $endDate, $generatedBy, $action) {
    ...
    echo '.assigned { color: #0275d8; }'; // Blue
    echo '.updated { color: #f0ad4e; }';  // Orange
    echo '.completed { color: #5cb85c; }'; // Green
    echo '.canceled { color: #d9534f; }';  // Red
    ...
    echo '<div class="report-header">Task Logs Report</div>';
    ...
    if ($action) {
        echo 'Action Filter: ' . ucfirst($action) . '<br>';
    }
    echo '</div>';
    ...
    if (count($data) > 0) {
        foreach ($data as $row) {
            ...
            echo '<td class="' . $actionClass . '">' . ucfirst($row['action']) . '</td>';
